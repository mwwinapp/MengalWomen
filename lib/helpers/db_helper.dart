...
// open the database
  ...
  Future<List<Member>> getAgeRange(int ageMin, int ageMax) async {
    var dBClient = await db;
    List<Map> maps = await dBClient.rawQuery("SELECT CASE WHEN DATE(substr(tblmembers.lastrenewal,7,4)||'-'||substr(tblmembers.lastrenewal,1,2)||'-'||substr(tblmembers.lastrenewal,4,2), '+1 year') <= date('now','start of month','+1 month','-1 day') THEN 'EXPIRED' ELSE 'ACTIVE' END AS Status FROM tblmembers WHERE Status='ACTIVE' AND age BETWEEN $ageMin AND $ageMax");
    List<Member> members = [];
    if (maps.length > 0) {
  ...
  Future<List<Member>> birthday() async {
    var dBClient = await db;
    List<Map> maps = await dBClient.rawQuery(
        "SELECT mid, lastrenewal, DATE(substr(tblmembers.lastrenewal,7,4)||'-'||substr(tblmembers.lastrenewal,1,2)||'-'||substr(tblmembers.lastrenewal,4,2), '+1 year') AS validity, lastname || ', ' || firstname || ' ' || CASE mi WHEN '' THEN '' ELSE mi || '.' END AS fullname, dob, barangay, CASE WHEN DATE(substr(lastrenewal,7,4)||'-'||substr(lastrenewal,1,2)||'-'||substr(lastrenewal,4,2), '+1 year') <= date('now','start of month','+1 month','-1 day') THEN 'EXPIRED' ELSE 'ACTIVE' END AS status FROM tblmembers WHERE substr(strftime('%Y-%m-%d',datetime(substr(dob, 7, 4) || '-' || substr(dob, 1, 2) || '-' || substr(dob, 4, 2))), 6, 5) = strftime('%m-%d',date('now', 'localtime')) ORDER BY fullname ASC");
    List<Member> members = [];
    if (maps.length > 0) {
      for (int i = 0; i < maps.length; i++) {
        members.add(Member.fromMap(maps[i]));
